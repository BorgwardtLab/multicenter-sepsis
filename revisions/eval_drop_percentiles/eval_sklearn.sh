#!/bin/bash
input_dir=results/hypersearch19_sklearn # path to hyperparameter search results e.g. hypersearch10_regression/
...
train_datasets=(eicu aumc hirid mimic) #eicu) 
eval_datasets=(eicu aumc hirid mimic) #(aumc hirid mimic eicu)
methods=(sofa qsofa sirs mews news) #lgbm sofa 
task=classification 
        ...
        for train_dataset in ${train_datasets[@]}; do
            for eval_dataset in ${eval_datasets[@]}; do
                ...
                python src/evaluation/subsampling.py \
                    ...
                    --subsampling-file config/splits/subsamples_${eval_dataset}_prev_0.188.json
