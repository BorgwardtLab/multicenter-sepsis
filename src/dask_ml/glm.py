# -*- coding: utf-8 -*-
""" Patched dask_ml models to better integrate with sklearn (scoring).
    When setting this patch up, we used dask_ml version 1.8.0
...
class _GLM(BaseEstimator):
    ...
    def fit(self, X, y=None):
        ...
        """
        # unpack df to array:
        X = self._check_array(to_array(X))
        y = to_array(y) 
        # patch for conforming with sklearn, assuming binary clf:
        if self.__class__.__name__ == 'LogisticRegression':
            if not hasattr(self, 'classes_'):
                print('Manually adding classes_')
...
class LogisticRegression(_GLM):
    ...
    def decision_function(self, X):
        ...
        """
        X_ = self._check_array(to_array(X))
    ...
    def predict_proba(self, X):
        ...
        """
        y_pred = sigmoid(self.decision_function(to_array(X))).reshape(-1,1)
        # patch: assuming binary clf:
    ...
    def score(self, X, y):
        ...
        score : float
            Mean accuracy score
        """
        return accuracy_score(to_array(y), self.predict(to_array(X)))
