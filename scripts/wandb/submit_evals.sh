...
for dataset in ${datasets[@]}; do
    ...
    sbatch -p gpu --gres=gpu:1 --cpus-per-task 4 --mem-per-cpu 6G --exclude=bs-gpu07 -J 'mcsepsis' --wrap "pipenv run python -m src.torch.eval_model_wandb ${wandb_run} --dataset ${dataset} --split ${split} --output ${pred_file}"
done
# MEMORY:
